...
export default function SongCarousel() {
  ...
  const fetchSongs = async () => {
    ...
    if (error) {
      ...
      // map Supabase data to your UI structure
      const formatted: Song[] = (data || []).map((s: any) => ({
        id: s.id,
        title: s.title,
        artist: s.artist ?? 'Unknown Artist',
        image: s.image_url ?? '/assets/dj1.jpg', // fallback image
        link: s.spotify_url,
      }))
      setSongs(formatted)
    }
      ...
      <div className="relative max-w-6xl mx-auto">
        <div className="embla overflow-hidden" ref={emblaRef}>
          <div className="embla__container flex px-6 py-12">
            {songs.map((song, i) => (
              <div key={song.id} className="flex-none px-3">
          ...
          {scrollSnaps.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                i === selectedIndex ? 'w-10 bg-pink-500' : 'w-2 bg-gray-500/50 hover:bg-gray-400/70'
              }`}
            />
          ))}
        </div>
...
// ===== SongCard unchanged, untouched =====
function SongCard({ song, priority = false }: { song: Song; priority?: boolean }) {
  return (
    <a
      href={song.link}
      target="_blank"
...
function Tag({ children, color = 'gray' }: { children: React.ReactNode; color?: 'pink' | 'gray' }) {
  const base = 'px-2 py-1 text-[10px] rounded-full transition-transform duration-200 group-hover:scale-105'
  return <span className={`${base} ${color === 'pink' ? 'bg-pink-600/20 text-pink-400' : 'bg-gray-800/60 text-gray-300'}`}>{children}</span>
}
